...
function addPeerToRoom(roomId, socketId, isHost = false) {
  ...
  if (room.peers.size === 0) {
    ...
    if (!currentHostInRoom) {
      ...
      console.log(`⚠️ ${socketId} tried to become host but ${room.host} is still active`)
    }
  }
  ...
  console.log(`📊 Room ${roomId}: host=${room.host}, peers=${Array.from(room.peers).join(', ')}`)
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (roomId, options = {}) => {
    try {
      ...
      // Join new room
      socket.join(roomId)
      const room = addPeerToRoom(roomId, socket.id, options.preferHost || false)
      ...
      console.log(`${socket.id} joined room ${roomId} as ${isHost ? 'host' : 'peer'}`)
  ...
  socket.on('host-reconnect', (data) => {
    try {
      ...
      console.log(`Host ${socket.id} rejoined room ${roomId} with file ${fileId}`)
