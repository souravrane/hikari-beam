      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div className="border-t pt-4">
          <div className="flex items-center justify-between text-sm text-gray-600">
            <div>
              <span className="font-medium">{p2p.peers.length}</span> peer(s) in room
            </div>
            {p2p.error && (
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
              ...
              {p2p.fileMetadata && (
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  <h4 className="font-medium text-blue-900">Downloading: {p2p.fileMetadata.name}</h4>
                  {p2p.transferProgress.has('download') && (
                    <div className="mt-2">
                      <div className="flex justify-between text-sm text-blue-700">
                        <span>Progress</span>
                        <span>{p2p.transferProgress.get('download').percentage.toFixed(1)}%</span>
                      </div>
                      <div className="w-full bg-blue-200 rounded-full h-2 mt-1">
                        <div 
                          className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                          style={{ width: `${p2p.transferProgress.get('download').percentage}%` }}
                        />
                      </div>
                    </div>
                  )}
          ...
          {p2p.isHost && p2p.fileMetadata && (
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  <span className="font-medium text-gray-700">File:</span> {p2p.fileMetadata.name}
                </div>
                {p2p.transferProgress.size > 0 && (
                  <div>
                    <h4 className="font-medium text-gray-700 mb-2">Peer Progress:</h4>
                    {Array.from(p2p.transferProgress.entries()).map(([peerId, progress]) => (
