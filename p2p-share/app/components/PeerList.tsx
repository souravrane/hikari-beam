    ...
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-1 sm:space-x-2">
                        <span className="font-medium text-gray-900 text-xs sm:text-sm truncate">
                          Peer {peer.id.slice(0, 6)}<span className="hidden sm:inline">{peer.id.slice(6, 8)}</span>
                        </span>
                        {peer.isHost && (
                      ...
                      <p className="text-xs text-gray-500 truncate">
                        <span className="sm:hidden">Joined {new Date(peer.joinedAt).toLocaleTimeString([], { timeStyle: 'short' })}</span>
                        <span className="hidden sm:inline">Joined {new Date(peer.joinedAt).toLocaleTimeString()}</span>
                      </p>
                    </div>
                  </div>

                  <span className={`px-1.5 sm:px-2 py-1 text-xs font-medium rounded border ${getStatusColor(connectionState)} flex-shrink-0 ml-2`}>
                    <span className="sm:hidden">{connectionState === 'connected' ? '✓' : connectionState === 'connecting' ? '...' : '✗'}</span>
                    <span className="hidden sm:inline">{getStatusText(connectionState)}</span>
                  </span>
                </div>
                  ...
                  <div className="mt-2 sm:mt-3 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-900">
                      <span className="truncate mr-2">
                        <span className="sm:hidden">{formatBytes(progress.receivedBytes || 0)}</span>
                        <span className="hidden sm:inline">{formatBytes(progress.receivedBytes || 0)} / {formatBytes(progress.totalBytes || 0)}</span>
                      </span>
                      {progress.speed > 0 && (
      ...
      {peers.length > 0 && (
        <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-200">
          <h4 className="text-xs sm:text-sm font-medium text-gray-900 mb-2">Room Statistics</h4>
          <div className="grid grid-cols-2 gap-3 sm:gap-4 text-xs text-gray-600">
            <div className="text-center sm:text-left">
              <span className="block">Total Peers</span>
              <span className="font-semibold text-gray-900 text-sm sm:text-base">{peers.length}</span>
            </div>
            <div className="text-center sm:text-left">
