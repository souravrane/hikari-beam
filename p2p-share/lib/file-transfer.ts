...
export function useP2PFileSender(
      ...
      try {
        switch (message.type) {
          ...
          case "ACK":
            ...
            state.ackedChunks = (state.ackedChunks || new Set()).add(
              message.data.index
            );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const avgProgress =
        totalPeers > 0
          ? (totalTransferred / (totalPeers * fileMetadata.totalChunks)) * 100
          : 0;
      const elapsedSeconds = (Date.now() - startTime.current) / 1000;
      const avgSpeed =
        elapsedSeconds > 0
          ? (totalTransferred * fileMetadata.chunkSize) / elapsedSeconds
          : 0;
...
export function useP2PFileReceiver(
  ...
  const [error, setError] = useState<string | null>(null);
  const [isActive, setIsActive] = useState(false);
  const [pendingMetadata, setPendingMetadata] = useState<FileMetadata | null>(
    null
  );
  ...
  const requestMissingChunks = useCallback(
    async (fileId: string) => {
      try {
        ...
        const hostConnection = Array.from(connections.values()).find((conn) =>
          conn.id.includes("host")
        );

        if (
          hostConnection?.dataChannel?.readyState === "open" &&
      ...
      try {
        switch (message.type) {
          ...
          case "CHUNK":
            ...
            // Send acknowledgment
            const hostConnection = Array.from(connections.values()).find(
            ...
            if (record) {
              ...
              const newProgress: TransferProgress = {
                ...
                eta: stats.eta,
                status:
                  record.receivedCount === record.metadata.totalChunks
                    ? "completed"
                    : "downloading",
              };
...
export function useP2PConnections(
  ...
  const [connections, setConnections] = useState<Map<string, PeerConnection>>(
    new Map()
  );
  const [error, setError] = useState<string | null>(null);

  const connect = useCallback(
    async (peerId: string, isInitiator = false) => {
      try {
        setError(null);
        ...
        if (isInitiator) {
          ...
          peerConnection.ondatachannel = (event) => {
            ...
            setupIncomingDataChannel(
              ...
              (error) =>
                console.error(
                  `Incoming data channel error from ${peerId}:`,
                  error
                )
            );
          };
